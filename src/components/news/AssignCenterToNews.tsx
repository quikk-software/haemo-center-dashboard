...
const AssignCenterToNews: FunctionComponent<AssignCenterToNewsProps> = ({initialCenters}) => {
    ...
    return <Grid container>
        <Grid item xs={12}>
            <FormControl fullWidth>
                <InputLabel id="center-select-checkbox-label">{t("news:center.label")}</InputLabel>
                <Select
                    ...
                    renderValue={(selected) => selected?.map(s => response.find(r => r.id === s)?.centerName)?.join(', ')}
                    MenuProps={MenuProps}
                >
                    {response.map(({id, centerName}) => (
                        <MenuItem key={id} value={id}>
                            <Checkbox checked={!!centers.find(centerId => centerId === id)}/>
